...
let audioEndedOffset = 0;
let audioEndedPerfTime = 0;
    ...
    try {
        ...
        if (songAudio) {
            songAudio.addEventListener('ended', () => {
                // currentTime freezes once playback ends, so switch to a
                // performance.now()-based clock picking up from that point,
                // otherwise trailing notes (after the audio's raw duration)
                // can never be scored and the match hangs indefinitely.
                audioHasEnded = true;
                audioEndedOffset = songAudio.currentTime * 1000;
                audioEndedPerfTime = performance.now();
            });
        }
        ...
        if (secondsLeft > 0) {
            ...
            if (songAudio) {
                ...
                songAudio.play().catch(error => {
                    console.warn("Chart audio could not start.", error);
                    // Fall back fully to the performance-timer clock instead
                    // of hanging on a stalled currentTime of 0.
...
function getPlaybackTime(timestamp) {
    if (songAudio && !audioPlaybackBlocked) {
        if (audioHasEnded) {
            // Audio's currentTime is frozen at its duration; keep the game
            // clock moving so trailing notes can still be hit/missed and
            // the match can actually finish.
            return audioEndedOffset + (timestamp - audioEndedPerfTime);
        }
